...
CREATE FUNCTION logging.GetLevelValue(@ExecutionId bigint = 0)
RETURNS tinyint
AS
BEGIN
  DECLARE @LevelValue tinyint;
  DECLARE @settingName nvarchar(256) = N'logging.level';
  ...
  IF @LevelValue IS NULL AND @ExecutionId > 0
   SET @LevelValue = logging.GetLevelValue(DEFAULT);
...
GRANT EXEC ON logging.GetLevelValue TO PUBLIC;
GO
