...
CREATE PROCEDURE logging.LogMessageS
 @tblLogs logging.LogsTable READONLY
AS
BEGIN
 SET NOCOUNT ON
 ...
 DECLARE @executeSQL nvarchar(max) = N'';

 BEGIN TRY
   SELECT @executeSQL += '
   execute logging.LogMessage @ExecutionId = '+cast(tl.ExecutionId as nvarchar(32))+',  @LevelName = '''+tl.LevelName+''', @Message = '''+tl.Message+''';'+char(13)
   FROM @tblLogs tl ;
...
GRANT EXEC ON logging.LogMessageS TO PUBLIC;
GO
